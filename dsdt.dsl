 ...
 *
 * Disassembly of dsdt.dat, Tue Jun 26 22:21:27 2018
 *
...
DefinitionBlock ("", "DSDT", 2, "MSFT  ", "MSFT    ", 0x00000000)
{
    ...
    External (_SB_.PCI0.GFX0.CLID, UnknownObj)                      // defined in ssdt6
    External (_SB_.PCI0.GFX0.DD1F, DeviceObj)
    External (_SB_.PCI0.GFX0.GLID, MethodObj)    // 1 Arguments     // defined in ssdt6
    ...
    External (IGDS, UnknownObj)                                         // defined in ssdt6
    ...
    External (ODV5, IntObj)                 // DEVICE OPERATION MODE ??? (values in {1, 2, ..., 5})
    ...
    Field (GNVS, AnyAcc, Lock, Preserve)
    {
        ...
        VFN9,   8,
        Offset (0x61),
        CPFM,   8,          // CURRENT PROCESSOR FAMILY?
        CPSP,   8,
        CPPT,   8,
        CPPS,   8,
        TBAL,   32,
    ...
    Scope (_SB)
    {
        ...
        Method (GGIV, 1, Serialized)            // GET [GIV]
        {
            Local0 = GGRP (Arg0)
            Local1 = GNMB (Arg0)
            Local2 = (GADR (Local0, 0x02) + (Local1 * 0x08))
        ...
        Method (GGII, 1, Serialized)        // GET GII
        {
            Local0 = GGRP (Arg0)
            Local1 = GNMB (Arg0)
            Local2 = (GADR (Local0, 0x02) + (Local1 * 0x08))
        ...
        Method (SGII, 2, Serialized)        // SET GII
        {
            Local0 = GGRP (Arg0)
            Local1 = GNMB (Arg0)
            Local2 = (GADR (Local0, 0x02) + (Local1 * 0x08))
    ...
    Scope (_SB.PCI0)
    {
        Device (HDAS)               // INTEL HIGH DEFINITION AUDIO [S] ???
        {
    ...
    Scope (_SB.PCI0)
    {
        Device (ISHD)           // INTEL INTEGRATED SENSOR SOLUTION
        {
    ...
    Scope (_SB)
    {
        Device (_SAN)                                   // SURFACE ACPI NOTIFY DRIVER
        {
            ...
            Name (DVER, Zero)
            Name (AVBL, Zero)                           // AVAILABLE (device availability flag)
            Name (RQBF, Buffer (0xFF)                   // REQUEST BUFFER
            {
                /* 0000 */  0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00,  // ........
                /* 0008 */  0x00, 0x00                                       // ..
            })
            CreateByteField (RQBF, 0x00, RSTS)          // REQUEST GSB STATUS
            CreateByteField (RQBF, 0x01, RTL)           // REQUEST GSB LENGTH
            CreateByteField (RQBF, 0x02, RCV)           // REQUEST COMMAND VALUE
            CreateByteField (RQBF, 0x03, RTC)           // target controller?                                   (RQSX.Arg0)
            CreateByteField (RQBF, 0x04, RTID)          // REQUEST [T] IDENTIFIER ???                           (fixed to 0x01)
            CreateByteField (RQBF, 0x05, RIID)          // REQUEST [I] IDENTIFIER ???                           (RQSX.Arg2)
            CreateByteField (RQBF, 0x06, RSNC)          // return value requested ???                           (RQSX.Arg4)
            CreateByteField (RQBF, 0x07, RCID)          // REQUEST COMMAND IDENTIFIER                           (RQSX.Arg1)
            CreateWordField (RQBF, 0x08, RCDL)          // REQUEST COMMAND DATA LENGTH
            CreateField (RQBF, 0x50, 0x07A8, RPLD)      // REQUEST PAYLOAD
            CreateByteField (RQBF, 0x00, VSTS)          // (ALIASES RSTS)
            CreateByteField (RQBF, 0x01, VLEN)          // (ALIASES RTL)
            CreateByteField (RQBF, 0x02, ASTS)          // (ALIASES RCV)
            CreateByteField (RQBF, 0x03, ALEN)          // (ALIASES RTC)
            Name (ETWB, Buffer (0x78){})                // ERROR [T] WRITE BUFFER
            CreateByteField (ETWB, 0x00, ETW0)          // GSB STATUS
            CreateByteField (ETWB, 0x01, ETW1)          // GSB LENGTH
            CreateByteField (ETWB, 0x02, ETW2)          // command value? (fixed t0 0x02 for ETWL)
            CreateByteField (ETWB, 0x03, ETW3)          // (either 0x01 or 0x02 depending on ETWL.Arg0)
            CreateByteField (ETWB, 0x04, ETW4)          // (ETWL.Arg1)
            Method (_STA, 0, NotSerialized)  // _STA: Status
            {
                Debug = "_SAN._STA\n"
                UDB0 ("_SAN._STA\n")
            ...
            {                                   // SHOULD GET CALLED WHEN OR01 IS AVAILABLE
                If ((Arg0 == 0x09))             // IF CONNECTED REGION IS "GenericSerialBus"
                {
                    AVBL = Arg1                 // UPDATE AVAILABILITY FLAG (1 -> connected)
                    Notify (ADP1, 0x80) // Status Change
                    Notify (BAT1, 0x81) // Information Change
                    If (CondRefOf (BAT2))
                    {
            ...
            {
                I2cSerialBusV2 (
                    0x0000,                 // SlaveAddress (address of this device on the bus)
                    ControllerInitiated,    // SlaveMode (initiated by the controller/driver,
                                            //            this means it's a slave device?)
                    0x000186A0,             // ConnectionSpeed (set to 100kHz)
                    AddressingMode7Bit,     // AddressingMode
                    "\\_SB._SAN"            // ResourceSource (indicates the I2C bus controller)
                    0x00,                   // reserved/unused in this revision
                    ResourceConsumer,       // reserved/unused in this revision
                    ,                       // optional/irrelevant in this context
                    Exclusive,              // Shared (exclusive vs. shared access)
                                            // VendorData (optional data provided to driver)
                    )
            })
            OperationRegion (OR01, GenericSerialBus, Zero /* offset */, 0x0100 /* num. command values */)
            ...
            {
                Return (RQSX (Arg0, Arg1, Arg2, Arg3, Arg4, 0x01))
            }
            ...
            /**
             * Generic I2C (/SSH?) request.
             *
             * Arguments:
             *   Arg0:
             *   Arg1:
             *   Arg2:
             *   Arg3: Request payload. Either Buffer or Integer (the latter is ignored).
             *   Arg4:
             *   Arg5:
             *
             * Returns status code (Integer) or Buffer.
             *
             * Status codes:
             *   0: Success, nothing to return.
             *   1: Communication system unavailable/uninitialized.
             *   2: Payload/Buffer to be sent is too big.
             *   3: Invalid argument: can only send buffer.
             *   4: SSH payload too big.
             *   5: SSH communication failure.
             *   6: I2C communication failure.
             *   7: Missing I2C header (returned payload too small).
             */
            Method (RQSX, 6, Serialized)
            {
                Local4 = One                                        // STATUS CODE (DEFAULT: CONNECTION UNAVAILABLE)
                If ((AVBL == One))                                  // IF DEVICE IS SET UP/AVAILABLE
                {
                    Local0 = ObjectType (Arg3)
                    If (((Local0 == Zero) || (Local0 == One)))      // Arg3 IS UNINITIALIZED OR INTEGER
                    {
                        Local1 = Zero                               // PAYLOAD SIZE (SEND NOTHING)
                    }
                    ElseIf ((Local0 == 0x03))                       // Arg3 IS BUFFER
                    {
                        Local1 = SizeOf (Arg3)                      // PAYLOAD SIZE (SEND BUFFER)
                        If ((Local1 > 0xF5))                        // CHECK IF PAYLOAD IS TOO BIG
                        {
                            Return (0x02)                           // ERROR: PAYLOAD TOO BIG
                        }
                    }
                    Else
                    {
                        Return (0x03)                               // ERROR: INVALID ARGUMENT
                    }                                               //        TYPE TO BE SENT IS NEITHER BUFFER NOR INTEGER

                    Local2 = (Local1 + 0x08)
                    RSTS = Zero                                     // STATUS
                    RTL = Local2                                    // transmission length?
                    RCV = Arg5                                      // command value?
                    RTC = Arg0                                      // target controller?
                    RTID = One
                    RIID = Arg2
                    RSNC = Arg4
                    RCID = Arg1                                     // COMMAND IDENTIFIER
                    RCDL = Local1                                   // COMMAND DATA LENGTH (PAYLOAD SIZE)
                    If ((Local1 != Zero))                           // CHECK IF PAYLOAD SIZE > 0
                    {
                        RPLD = Arg3                                 // SET PAYLOAD
                    }

                    VSTS = One                  // SET STATUS TO ONE TO LATER CHECK COMM SUCCESS
                    RQBF = SAN0 = RQBF /* \_SB_._SAN.RQBF */    // I2C ROUNDTRIP USING SAN0 COMMAND VALUE
                    // SEE ACPI SPEC: Declaring and Using IPMI Request and Response Buffer
                    ...
                    Else
                    {
                        Local4 = 0x06                           // STATUS: COMMUNICATION FAILED
                        Debug = "RQST: I2C communication failed\n"
                        UDB0 ("RQST: I2C communication failed\n")
                    }

                    If ((ObjectType (Local4) == One))           // IF Local4 IS INTEGER
                    {
                        If ((Local4 != Zero))                   // IF ERROR HAS OCCURRED
                        {
                            Local5 = Package (0x04){}
                            Local5 [0x00] = Arg0
                            Local5 [0x01] = Arg1
            ...
            /**
             * Write formatted error string to I2C device.
             *
             * Takes a format string and values to be formatted with it.
             * The formatted output is written to ETWB, including a header.
             *
             * Arguments:
             *   Arg0: ?
             *   Arg1: ?
             *   Arg2: Format string, using %i to denote i-th value in package.
             *   Arg3: Package containing values to be displayed with format string.
             */
            Method (ETWL, 4, Serialized)                // ERROR [T] WRITE LINE
            {
                If ((AVBL == One))
                {
                    ToBuffer (Arg2, Local2)                 // REINTERPRET FORMAT STRING AS BUFFER
                    Local0 = SizeOf (Arg2)                  // LENGTH OF FORMAT STRING (WITHOUT NULL)
                    Local1 = Zero                           // FORMAT STRING INDEX (RUNNING)
                    Local6 = 0x05                           // BUFFER INDEX (RUNNING)
                    While (((Local1 < Local0) && (Local6 < 0x78)))
                    {
                        Local3 = DerefOf (Local2 [Local1])
                        Local1++
                        If ((Local3 == 0x25))               // IF CURRENT FORMAT STRING CHAR IS `%`
                        {
                    ...
                    Local6 -= 0x02                          // SUBTRACT (I2C?) HEADER LENGTH
                    ETW0 = 0x00                             // GSB STATUS
                    ETW1 = Local6                           // GSB LENGTH
                    ETW2 = 0x02                             // command value?
                    If ((Arg0 == One))
                    {
                        ETW3 = 0x01
                    }
            ...
            /**
             * Build a package for ETWL, containing a single value.
             */
            ...
            /**
             * Build a package for ETWL, containing 2 values.
             */
             ...
             * BUGGY (and unused)
             *
             * Build a package for ETWL, containing 3 values.
             */
            Method (EWP3, 3, NotSerialized)
            {
                Local0 = Package (0x02){}       // BUG: Package length should be 3
                Local0 [0x00] = Arg0
                Local0 [0x01] = Arg1
                Local0 [0x02] = Arg2
                Return (Local0)
             ...
             * BUGGY (and unused)
             *
             * Build a package for ETWL, containing 4 values.
             */
            Method (EWP4, 4, NotSerialized)
            {
                Local0 = Package (0x02){}       // BUG: Package length should be 4
                Local0 [0x00] = Arg0
                Local0 [0x01] = Arg1
                Local0 [0x02] = Arg2
                Local0 [0x03] = Arg3
    ...
    Device (ADP1)
    {
        ...
        Method (ACST, 0, Serialized)                        // GET AC STATUS
        {
            Local2 = Zero                                   // DEFAULT TO AC DISCONNECTED
            Local0 = \_SB._SAN.RQST (0x02, 0x0D, One, Zero, One)
            If ((ObjectType (Local0) != 0x03))              // IF RQST RETURNS NON-BUFFER
            {                                               // RETURN AC DISCONNECTED (DEFAULT)
    ...
    Scope (_SB.PCI0.I2C2)
    {
        Device (CAMF)                   // MICROSOFT CAMERA FRONT
        {
            Name (_ADR, Zero)  // _ADR: Address
        ...
        Device (SKC1)               // INTEL CONTROL LOGIC
        {
            Name (_ADR, Zero)  // _ADR: Address
    ...
    Scope (_SB.PCI0.I2C3)
    {
        Device (CAMR)                   // MICROSOFT CAMERA REAR
        {
            Name (_ADR, Zero)  // _ADR: Address
        ...
        Device (SKC0)                   // INTEL CONTROL LOGIC
        {
            Name (_ADR, Zero)  // _ADR: Address
            Name (_HID, "INT3472")  // _HID: Hardware ID
            Name (_CID, "INT3472")  // _CID: Compatible ID
    ...
    Scope (_SB.PCI0.I2C3)
    {
        Device (CAM3)                   // MICROSOFT IR CAMERA FRONT
        {
            Name (_ADR, Zero)  // _ADR: Address
        ...
        Device (SKC2)               // INTEL CONTROL LOGIC
        {
            Name (_ADR, Zero)  // _ADR: Address
            Name (_HID, "INT3472")  // _HID: Hardware ID
            Name (_CID, "INT3472")  // _CID: Compatible ID
    ...
    Scope (_SB)
    {
        Device (BAT1)                                       // PRIMARY BATTERY (TOP)
        {
            ...
            Method (_STA, 0, Serialized)  // _STA: Status
            {
                ...
                If ((ObjectType (Local0) != 0x03))          // IF REQUEST RETURNS NON-BUFFER OBJECT
                {
                    If ((Local0 == One))                    // ONE MEANS AVBL HAS NOT BEEN SET
                    {
                ...
                ElseIf ((SizeOf (Local0) == 0x04))          // IF BUFFER CONTAINS 4 BYTES
                {
                    CreateDWordField (Local0, Zero, BAST)   // REINTERPRET BUFFER AS DWORD
                    UDB2 ("BAST:Battery%0 Attached=%1X\n", One, BAST)
                    If ((BAST & 0x10))                      // CHECK OF BATTERY IS ATTACHED
                    {
    ...
    Scope (_SB)
    {
        Device (BAT2)                                           // SECONDARY BATTERY (BASE)
        {
            Name (_HID, EisaId ("PNP0C0A") /* Control Method Battery */)  // _HID: Hardware ID
            Name (_UID, 0x02)  // _UID: Unique ID
            ...
            Method (_STA, 0, Serialized)  // _STA: Status
            {
                ...
                If ((ObjectType (Local0) != 0x03))              // IF RQST RETURNS NON-BUFFER OBJECT
                {
                    If ((Local0 == One))                        // ONE MEANS AVBL HAS NOT BEEN SET
                    {                                           // ACTUALLY RETURNS BATTERY NOT PRESENT
                        UDB0 ("SSH -> SAM communication is not ready. Returning a hardcoded \'battery present\' state to OS\n")
                    }
                    Else
                    {
    ...
    Scope (_SB)
    {
        Device (LID0)
        {
            ...
            Name (BASE, 0xFF)                               // BASE ATTACHMENT STATE ???
            Name (_DEP, Package (0x01)  // _DEP: Dependencies
            {
                _SAN
            })
            ...
            Method (GLSS, 0, Serialized)                    // GET LID STATUS
            {
                If ((GGIV (0x02020017) == One))             // GET [GIV]
                {
                    SGII (0x02020017, One)                  // SET [GII]
                    LIDS = Zero
                }
                Else
                {
                    SGII (0x02020017, Zero)                 // SET [GII]
                    LIDS = One
                }
            ...
            Method (GBOS, 0, NotSerialized)                 // GET [BOS] (UPDATE BASE)
            {
                BASE = 0xFF
                Local2 = Zero
                Local1 = ^^_SAN.RQST (0x11, 0x0D, Zero, Zero, One)          // GET BASE STATUS
                If ((ObjectType (Local1) != 0x03)){}        // Local1 IS BUFFER
                ElseIf ((SizeOf (Local1) == One))           // Local1 IS SINGLE BYTE (buf. len is 1)
                {
            ...
            {
                If ((BASE == 0xFF))                         // IF BASE STATUS UNINITIALIZED ???
                {
                    ULID ()                                 // UPDATE LID STATUS
                    Return (One)                            // RETURN LID OPEN
                }

                Local0 = GLSS ()                            // GET (RAW) LID STATUS
                UOD5 (LIDS, BASE)                           // UPDATE DEVICE OPERATION MODE ???
                If ((BASE != One))                          // IF BASE IS NOT ATTACHED ???
                {
                    Return (One)                            // RETURN LID OPEN
                }

                Return (LIDS) /* External reference */      // RETURN LID STATUS
            }

            Method (ULID, 0, NotSerialized)                 // UPDATE LID STATUS
            {
                Local0 = GLSS ()                            // GET (RAW) LID STATUS
                GBOS ()                                     // UPDATE BASE STATUS
                If ((BASE != One))                          // IF BASE IS NOT ATTACHED ???
                {
                    Local0 = One                            // SET LOCAL LID STATUS TO OPEN
                }

                If (IGDS)                                   // IF EXTERNAL VALUE (ssdt6)
                {                                           // something with graphics state ???
                    If (^^PCI0.GFX0.GLID (Local0))          // IF EXTERNAL MEHTOD CALL... (ssdt6)
                    {
                        ^^PCI0.GFX0.CLID |= 0x80000000 /* External reference */
                    }
                }

                Notify (LID0, 0x80) // Status Change        // NOTIFY LID STATUS CHANGE
            }
        }
    }
    ...
    Scope (_SB)
    {
        Device (WSLT)                       // GPIO LAPTOP OR SLATE INDICATOR ???
        {
            Method (_HID, 0, NotSerialized)  // _HID: Hardware ID
            {
                Return ("MSHW0005")
        ...
        Device (WSID)                       // [WINDOWS] SURFACE INTEGRATION [DEVICE]
        {
            Method (_HID, 0, NotSerialized)  // _HID: Hardware ID
            {
                Return ("MSHW0107")
        ...
        Device (BIND)               // BUTTON INTEGRATION DRIVER (?)
        {
        ...
        Device (TSML)                           // TOUCH SERVICING ML
        {
            Method (_HID, 0, NotSerialized)  // _HID: Hardware ID
            {
                If (((EBID & 0x40) == Zero))
        ...
        Method (UOD5, 2, Serialized)            // UPDATE ODV5 (OPERATIONAL MODE)?
            // Arg0: LID STATE
            // Arg1: BASE STATE
        {
            If ((Arg1 == Zero))                 // IF BASE DETACHED ???
            {
                Local0 = 0x03                   // TABLET OPERATION MODE ???
            }
            ElseIf ((Arg1 == One))              // IF BASE ATTACHED ???
            {
                If ((Arg0 == Zero))             // IF LID CLOSED
                {
                    Local0 = 0x02               // CLOSED OPERATION MODE ???
                }
                Else
                {
                    Local0 = One                // NORMAL OPERATION MODE ???
                }
            }
            ElseIf ((MANF == Zero))             // IF SOME VALUE IN SYSTEM MEMORY...
            {
                Local0 = One
            }
            ElseIf ((Arg0 == Zero))             // IF LID CLOSED, BASE UNINITIALIZE
            {
                Local0 = 0x04
            }
            Else                                // LID OPEN/UNINITIALIZED, BASE UNINITIALIZED
            {
                Local0 = 0x05
            }
        ...
        Device (CSRP)                           // SURFACE RADIO MONITOR
        {
            Method (_HID, 0, NotSerialized)  // _HID: Hardware ID
            {
                Return ("MSHW0131")
        ...
        Device (SHPS)                           // SURFACE HOT-PLUG [S?]
        {
    ...
    Method (PNOT, 0, Serialized)            // POWER NOTIFICATIONS ???
    {
        If ((TCNT > One))
        {
            If ((PC00 & 0x08))
    ...
    Scope (_SB.PCI0.ISP0)           // INTEL IMAGING SIGNAL PROCESSOR 2500
    {
        Method (_DEP, 0, NotSerialized)  // _DEP: Dependencies
        {
            If (((S0ID == One) || (OSYS >= 0x07DF)))
    ...
    Scope (_SB)
    {
        Device (PEPD)               // INTEL POWER ENGINE PLUG-IN
        {
            ...
            Method (_DSM, 4, Serialized)  // _DSM: Device-Specific Method
            {
                // SEE: https://www.uefi.org/sites/default/files/resources/Intel_ACPI_Low_Power_S0_Idle.pdf
                If ((Arg0 == ToUUID ("c4eb40a0-6cd2-11e2-bcfd-0800200c9a66")))
                {
                    If ((Arg2 == 0x00))             // Enum Functions
                    {
                        Return (Buffer (One)
                        {
                             0x7F                                             // .
                    ...
                    If ((Arg2 == 0x01))             // Get Device Constraints
                    {
                        If ((S0ID == Zero))
                        {
                            Return (Package (0x00){})
